...
//        print('snapshot : ${snapshot.data}');
        if (!snapshot.hasData)
          return Center(child: Text('No se encontraron vistas.'));
        return Column(children: snapshot.data.map((vista) => vista).toList());
      },
    );
  }
...
class Area extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (chk.calcs['bloques'][bloqueId]['areas'][area['identificador']]
            ['muestra'] ==
        1) {
      return Container(
        child: Column(
...
//        print('snapshot : ${snapshot.data}');
        ...
        return Column(
            children: snapshot.data.map((pregunta) {
          return Container(
              padding: const EdgeInsets.only(bottom: 10),
...
//        child: Text(''),
    ...
    if (tipo != 'sub') {
      var nomResp = pregChk['nomResp'] == null
          ? ''
          : parseHtmlString('${pregChk['nomResp']}');
