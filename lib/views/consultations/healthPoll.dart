        ...
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.done:
            ...
            for (int i = 0; i < elementos.length; i++) {
              preguntas.add(PreguntaHealth(
                pregunta: elementos[i],
                setResp: setResp,
              ));
            }

            preguntas.add(Container(
                  ...
                  Expanded(
                    ...
                    child: RaisedButton(
                      ...
                      onPressed: () async {
                        ...
                        String fechaUltimoEnv =
                            userData.getString('lastDatePoll_${widget.pollId}');
                        DateTime now = DateTime.now();
                        String fechaAct = '${now.year}-${now.month}-${now.day}';

                        print(
                            "fechaUltimoEnv: ${fechaUltimoEnv}, fechaAct: $fechaAct");
...
//                          fechaUltimoEnv = fechaAct;
                        if (fechaAct == fechaUltimoEnv) {
                          emergente(
                              ...
                              actions: [],
                              content: Text(
                                  'Esta encuesta sólo puede ser contestada una vez al día'));
                          return;
                        }
                        ...
                        if (envOk) {
                          userData.setString(
                              'lastDatePoll_${widget.pollId}', fechaAct);
                        } else {
                          emergente(
                              context: context,
                              actions: [],
  ...
  getPregs() async {
    List preguntasAPI = await getDatos(
        opt: 'polls/questions/?poll=${widget.pollId}',
        varNom: 'polls',
        imprime: false);
...
class PreguntaHealthState extends State<PreguntaHealth> {
  ...
  @override
  Widget build(BuildContext context) {
    widget.setResp(llave: '${widget.pregunta['id']}', valor: activo);
          ...
          Expanded(
            ...
            child: Checkbox(
                ...
                onChanged: (a) {
                  ...
                  widget.setResp(
                      llave: '${widget.pregunta['id']}', valor: activo);
                }),
          )
        ],
      ),
