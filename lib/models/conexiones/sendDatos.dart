...
//    }
  ...
  getUserConsultationsChecklist(bool creadoOffline, bool offline) async {
    ...
    sql =
        'SELECT * FROM UsersConsultationsChecklist WHERE $wCreadoOffline AND $wOffline';
...
//    print('CTES = $uccs');
    ...
    for (int i = 0; i < uccs.length; i++) {
      ...
      ucc['ucc'] = c;
      print('elemID: ${c['id']}');
      ucc['visitas'] = await getVis(
          elemId: c['id'], creadoOffline: false, offline: false, type: 'cons');
...
//    }
  ...
  getTargetsElem(
      bool creadoOffline, bool offline, int dimensionesElemId) async {
    String sql;
    List targetsElem = [];
    String wCreadoOffline;
    String wOffline;
    ...
    if (dimensionesElemId != null) {
      sql =
          'SELECT * FROM TargetsElems WHERE dimensionesElemId = $dimensionesElemId';
    } else {
...
//    print('CTES = $trgtsElems');
    ...
    for (int i = 0; i < trgtsElems.length; i++) {
      ...
      Map trgtElem = Map();
      trgtElem['trgtElem'] = c;
      trgtElem['visitas'] = await getVis(
          elemId: c['id'], creadoOffline: false, offline: false, type: 'trgt');
  ...
  getRespuestasVisita(int vId) async {
    ...
    for (int i = 0; i < respuestasDB.length; i++) {
      ...
      if (respuesta['respuesta'] == 'spatial') {
        List problemsDB = await db.query(
            "SELECT * FROM problems WHERE respuestasVisitaId = ${respuesta['id']}");
        problemsDB ??= [];
        List problems = [];
        for (int j = 0; j < problemsDB.length; j++) {
          Map problem = Map.from(problemsDB[j]);
          problem['photo64'] = problem['photo'] != null
              ? await getFileToStr(archivo: problem['photo'], subdir: 'fotos')
              : '';

          List points = await db.query(
              "SELECT * FROM Points WHERE problemsId = ${problem['id']}");
          points ??= [];
...
//    print('RESPUESTAS $respuestas');
  ...
  getMult(int vId) async {
    List mResult =
        await db.query("SELECT * FROM Multimedia WHERE visitasId = $vId");
    List mult = [];
    mult ??= [];
    mResult ??= List();
...
//    print("$archivo,$subdir");
  ...
  getPolls() async {
    ...
    List polls =
        await db.query("SELECT * FROM UsersQuickPoll WHERE usersId = $usrId");
  ...
  delVisita(visita) async {
    try {
      await db.query(
          'DELETE FROM RespuestasVisita WHERE visitasId = ${visita['visita']['id']}');
      await db.query(
          'DELETE FROM Multimedia WHERE visitasId = ${visita['visita']['id']}');
